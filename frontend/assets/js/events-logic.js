...
const DEFAULT_IMAGE = '../assets/images/placeholder.jpg'; // Placeholder genérico (opcional)
...
// --- LÓGICA PRINCIPAL ---
/**
 * Função que verifica o status de login e redireciona (ou inscreve).
 */
...
async function fetchAndRenderEvents(container) {
    ...
    try {
        // ⭐ CORREÇÃO APLICADA: Adicionando o prefixo '/evento' ⭐
...
function renderEvents(container, events) {
    events.forEach(event => {
        ...
        // 1.  ✅  Lógica Condicional para o Visual do Card
...
<p>${escapeHtml(event.titulo)}</p>
</div>
`;

        const imageContainerClass = imageUrl ? '' : 'no-image'; // Adiciona a classe extra se não houver imagem
