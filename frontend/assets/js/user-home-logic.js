...
/*
================================================================
FUNÇÕES DE NAVEGAÇÃO E EVENTOS DE AÇÃO
================================================================
*/
...
/*
================================================================
HOME (Visão Geral da Conta e Próximos Eventos)
================================================================
*/
export async function renderHomeContent(container) {

    // Conteúdo Estático da Home
    container.innerHTML = '<h2><i class="ph ph-sparkles"></i> Visão Geral</h2>';

    const userData = {
        email: document.body.dataset.userEmail || 'N/A',
        phone: document.body.dataset.userPhone || 'Não informado',
        joinedDate: document.body.dataset.userCreatedAt
...
<div class="data-item"><strong>E-mail:</strong> ${userData.email}</div>
<div class="data-item"><strong>Telefone:</strong> ${userData.phone}</div>
<div class="data-item"><strong>Membro Desde:</strong> ${userData.joinedDate}</div>
</div>
`;
...
// Carrega eventos aprovados para exibição na Home do Dashboard
async function loadEvents() {

    try {
        // Rota GET: /api/evento/eventos
...
<a href="eventos.html" class="text-center" style="color: #FBBF24; display:block; margin-top:15px;">
Ver Todos
</a>
`;
            }
...
`;
    ...
    try {
        // Rota GET: /api/evento/minhas-inscricoes
...
/*
================================================================
SUGERIR EVENTO (Criação e Listagem de Meus Eventos)
================================================================
*/
...
`;
    ...
    // Carrega a lista de eventos criados por este usuário
    fetchMyCreatedEventsForEditing(qs('#my-created-events-list'));
}
...
async function handleSuggestEvent(e) {
    ...
    try {
        ...
        // Recarrega a lista de eventos criados na aba
        fetchMyCreatedEventsForEditing(qs('#my-created-events-list'));
...
`;
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            // Rota POST: /api/mapa/pontos-coleta
            const resp = await fetchJson('mapa/pontos-coleta', {
                method: 'POST',
                body: JSON.stringify(payload)
            });

            resultDiv.innerHTML = `<p style="color: #10B981;">${resp.message || 'Ponto adicionado com sucesso! (Aguarde a atualização do mapa)'}</p>`;
            form.reset();

        } catch (err) {
            console.error('Erro ao adicionar ponto:', err);
            resultDiv.innerHTML = `<p style="color: #DC2626;">Erro: ${escapeHtml(err.message || 'Falha no servidor.')}</p>`;
        } finally {
            btn.disabled = false;
            btn.innerHTML = '<i class="ph ph-plus-circle"></i> Adicionar Ponto';
        }
...
`;
    ...
    try {
        // Rota GET: /api/evento/eventos/meus
...
<button class="btn btn-sm edit-event-btn" data-event-id="${event.id_evento}" style="background-color: #3B82F6; padding: 5px 10px; font-size: 0.9em;">
Editar
</button>
<button class="btn btn-sm delete-event-btn" data-event-id="${event.id_evento}" style="background-color: #DC2626; padding: 5px 10px; font-size: 0.9em;">
Apagar
</button>
</div>
</div>
`).join('');
        ...
        // Anexa listeners de Edição e Deleção
        ...
        if (targetContainer)
            targetContainer.innerHTML = `<p style="color: red;">Erro ao carregar eventos criados: ${error.message}</p>`;
